...
class StateManager {
  ...
  /**
   * BELANGRIJK:
   * - Zorgt dat elk slot altijd de nieuwste "shape" heeft (qa/systemData/etc.)
   * - Preserveert linkedSourceId
   * - Voorkomt dat linked input slots qa/systemData missen (waardoor labels/scores verdwijnen)
   */
      ...
      sheet.columns.forEach((col) => {
        ...
        col.slots = col.slots.map((slot) => {
          ...
          const mergedSlot = {
            ...clean,
            ...s,
            // merge nested objects zodat keys nooit ontbreken
            qa: { ...clean.qa, ...(s.qa || {}) },
            systemData: { ...clean.systemData, ...(s.systemData || {}) },

            // expliciet behouden (soms verdwijnen keys bij oudere saves)
  ...
  /**
   * Output-map dat overal gebruikt wordt (UI + link select + export).
   * Keys: OUT1, OUT2, ...
   */
  getAllOutputs() {
    const map = {};
